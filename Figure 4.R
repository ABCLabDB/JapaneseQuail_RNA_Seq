...
data <- fread("./Data/Figure4_data/Figure4_data.txt")
...
Annotation <- fread("./Data/Figure4_data/Figure4_data.txt")
## Loading featureCount data
meta_data <- data.frame(fread("./Data/meta_data.txt", sep = "\t",header=T))


fileList <- paste0("./Data/Quantification_quail/",meta_data$Run, '.txt')
...
for(i in 1:length(gene_list)){
  ...
  # Drawing
  ggplot() +
    geom_jitter(data = plot_data, aes(x = Sex, y = Expression, color = Tissue), width = 0.1, height = 0, size = 5, alpha = 0.5) + 
    geom_line(data = plot_data_summary, aes(x = Sex, y = Mean, group = Tissue, color = Tissue), size = 1.5) +
    geom_errorbar(data = plot_data_summary, aes(x = Sex, ymin = Mean - SE, ymax = Mean + SE, group = Tissue, color = Tissue), width = 0.2) +
    scale_color_manual(values = c("#FF7A01", "#70398C")) + 
    theme_classic() +
