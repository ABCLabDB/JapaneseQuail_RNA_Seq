...
meta_data <- data.frame(fread("./Data/Figure3_data/meta_data_O_T.txt", sep = "\t",header=T))
...
results <- fread("./Data/Figure3_data/Result_O_T.txt", head = T, sep = "\t") %>%
...
O_T_data <- results %>%
  ...
  dplyr::select(Gene.name, Gene.type, Chromosome.scaffold.name, negative_P_value, logFC, logFC_group, label, color_by_FDR, FDR) 
...
### Load data ###
load(file="./Data/Figure3_data/O_T.RData")
...
for(i in 1:length(temp_name)){
  ...
  p2 <- ggplot(filtered_data, aes(x = Tissue, y = Expression, fill = Tissue)) +
    ...
    geom_jitter(aes(color = Tissue),alpha = 0.5, size = 5) +  
    scale_fill_manual(values = c("Ovary" = "#F5D042", "Testis" = "#0A174E")) +  
    scale_color_manual(values = c("Ovary" = "#F5D042", "Testis" = "#0A174E")) +  
    theme_classic2()+
    theme(
      legend.position = "none",
      axis.title.x = element_blank(),
...
data <- fread("./Data/Figure3_data/Result_O_T.txt")
...
ggplot(data, aes(x = Chromosome.scaffold.name, y = logFC, fill = Chromosome.scaffold.name)) +
  ...
  theme_classic() +
  geom_hline(yintercept =  mean(data$logFC), linetype="dashed",color = "red", size = 1.3,alpha=0.8) + 
...
lines <- readLines("./Data/Figure3_data/Male_up.txt")
...
data <- fread("./Data/Figure3_data/Male_up_bar_plot_data.txt")
...
Zplot <- ggplot(data, aes(x=Term, y=-log10(data$PValue), fill=Term)) +
  geom_bar(stat="identity", colour="black", size=1,width=1) +  
...
lines <- readLines("./Data/Figure3_data/Female_up.txt")
...
data <- fread("./Data/Figure3_data/Female_up_bar_plot_data.txt")
...
Zplot <- ggplot(data, aes(x=Term, y=-log10(data$PValue), fill=Term)) +
  geom_bar(stat="identity", colour="black", size=1,width=0.95) +  생성
  scale_fill_manual(values = palette) +  
  theme_classic() +
  scale_y_continuous(expand = c(0,0)) +
  geom_hline(yintercept=-log10(0.01),linetype=2,size=1.3) +
